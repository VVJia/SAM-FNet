...
def test_single_volume(image, label, net, classes, multimask_output, patch_size=[256, 256], input_size=[224, 224],
                       test_save_path=None, case=None, results=None):
    image, label = image.squeeze(0).cpu().detach().numpy(), label.squeeze(0).cpu().detach().numpy()
    orign_image = copy.deepcopy(image)
    x, y = image.shape[0:2]
    if x != input_size[0] or y != input_size[1]:
        image = zoom(image, (input_size[0] / x, input_size[1] / y, 1.0), order=3)  # previous using 0
    new_x, new_y = image.shape[0], image.shape[1]  # [input_size[0], input_size[1]]
    ...
    if test_save_path is not None:
        img = Image.fromarray(orign_image.astype(np.uint8))
        prd = Image.fromarray((prediction*255).astype(np.uint8))
        lab = Image.fromarray((label*255).astype(np.uint8))

        # 可视化结果
        # img.save(os.path.join(test_save_path, case + "_ori.jpg"))
        # prd.save(os.path.join(test_save_path, case + "_prd.jpg"))
        # lab.save(os.path.join(test_save_path, case + "_gt.jpg"))

        results[case] = {"image": img, "pred": prd, "label": lab}
